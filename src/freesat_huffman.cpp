		...
		for (j = 0; j < 127; j++) {//was 255
			...
			for (k = 0; k < table_size[i][j]; k++) {
				fprintf(fp, "    { 0x%08x, %2i, %3d}, /* %4d '%s' */\n",
						tables[i][j][k].value, tables[i][j][k].bits,
						tables[i][j][k].next, line++, escape_char(
								tables[i][j][k].next));
			}
		}
		j = 127;
...
static void load_file(int tableid, char *filename) {
	...
	if ((fp = fopen(filename, "r")) != NULL) {
		log_message(TRACE, "loading table %d filename \"%s\"", tableid + 1,
				filename);

		while (fgets(buf, sizeof(buf), fp) != NULL) {
			from = binary = to = NULL;
			int elems = sscanf(buf, "%a[^:]:%a[^:]:%a[^:]:", &from, &binary,
					&to);
			if (elems == 3) {
				...
				tables[tableid][from_char] = (struct fsattab *) realloc(
						tables[tableid][from_char], (i + 1)
								* sizeof(tables[tableid][from_char][0]));
		...
		log_message(ERROR, "cannot load \"%s\" for table %d", filename, tableid
				+ 1);
	}
}
		...
		do {
			...
			if (lastch == ESCAPE) {
				...
				if (p >= uncompressed_len) {
					uncompressed_len += 10;
					uncompressed = (u_char *) realloc(uncompressed,
							uncompressed_len + 1);
				}
			...
			if (found) {
				...
				tlen = snprintf(temp, sizeof(temp), "[%02x][%02x][%02x][%02x]",
				...
				do {
					...
					tlen += snprintf(temp + tlen, sizeof(temp) - tlen,
							"[%02x]", value & 0xff);
				...
				log_message(ERROR, "missing table %d entry \"%s\"",
						tableid + 1, uncompressed);
...
void dump_compressed(u_char *src, uint size) {
	...
	do {
		...
		tlen += snprintf(temp + tlen, sizeof(temp) - tlen, "%02x, ", value
				& 0xff);
	} while (tlen < sizeof(temp) - 6 && byte < size);

	log_message(ERROR, "table %d len =%d uncompressed=\"%s\"", src[1], size,
			temp);
}
			...
			if (found) {
				if (nextCh != STOP && nextCh != ESCAPE) {
					if (p >= alloc_size) {
						...
						add_to_stat("freesathuffman.uncompressed", ALLOC_INCR);
						uncompressed = (unsigned char *) realloc(uncompressed,
								alloc_size);
					}
					uncompressed[p++] = nextCh;
				}
				...
				log_message(
						ERROR,
						"entry missing in huffman table %d for Freesat value=%x0 uncompressed=\"%s\"",
						src[1], value, uncompressed);
				return uncompressed;
			}
