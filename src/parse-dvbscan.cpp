	...
	while (NULL != (token = strtok(0, DELIMITERS))) {
		...
		switch((enum __extflags)arg++) {
			...
			case filter_timeout:
				...
				break;
			case fe_type:
				flags->fe_type = (fe_type_t)txt_to_fe_type(token);
				break;				
			case list_idx:
...
int dvbscan_parse_tuningdata(const char * tuningdata, struct w_scan_flags * flags) {
	...
	while (fgets(buf, MAX_LINE_LENGTH, initdata) != NULL) {
		...
		if (copy == NULL) {
			log_message(ERROR, "Could not allocate memory.");
			exit(1);
			}
		...
		switch (toupper(token[0])) {
			...
			case '#':
				...
				continue;
			default:
				log_message(ERROR, "could not parse %s", tuningdata);
				return 0; // err
			}
		...
		switch (tn->type) {
			...
			case FE_ATSC:
				...
				break;
			default:
				log_message(ERROR, "could not parse '%s' - undefined fe_type", buf);
				return 0; // err
			}

		while (NULL != (token = strtok(0, DELIMITERS))) {
			switch ((enum __dvbscan_args)arg++) {
				case qpsk_frequency:
				case qam_frequency:
				case ofdm_frequency:
				case atsc_frequency:
				...
				case qpsk_polarization:
					tn->param.u.qpsk.polarization = (fe_polarization_t)txt_to_qpsk_pol(token);
					break;
				case qpsk_symbol_rate:
					tn->param.u.qpsk.symbol_rate = strtoul(token, NULL, 10);
					break;
				case qpsk_fec_inner:
					tn->param.u.qpsk.fec_inner = (fe_code_rate_t)txt_to_qpsk_fec(token);
					count++;
					break;
				case qpsk_rolloff:
					tn->param.u.qpsk.rolloff = (fe_rolloff_t)txt_to_qpsk_rolloff(token);
					break;
				case qpsk_modulation:
					tn->param.u.qpsk.modulation_type = (fe_modulation_t)txt_to_qpsk_mod(token);
					break;
				case qam_symbol_rate:
					tn->param.u.qam.symbol_rate = strtoul(token, NULL, 0);
					break;
				case qam_fec_inner:
					tn->param.u.qam.fec_inner = (fe_code_rate_t)txt_to_qam_fec(token);
					break;
				case qam_modulation:
					tn->param.u.qam.modulation = (fe_modulation_t)txt_to_qam_mod(token);
					count++;
					break;
				case ofdm_bandwidth:
					tn->param.u.ofdm.bandwidth = (fe_bandwidth_t)txt_to_ofdm_bw(token);
					break;
				case ofdm_fec_high_priority:
					tn->param.u.ofdm.code_rate_HP = (fe_code_rate_t)txt_to_ofdm_fec(token);
					break;
				case ofdm_fec_low_priority:
					tn->param.u.ofdm.code_rate_LP = (fe_code_rate_t)txt_to_ofdm_fec(token);
					break;
				case ofdm_modulation:
					tn->param.u.ofdm.constellation = (fe_modulation_t)txt_to_ofdm_mod(token);
					break;
				case ofdm_transmission_mode:
					tn->param.u.ofdm.transmission_mode = (fe_transmit_mode_t)txt_to_ofdm_transmission(token);
					break;
				case ofdm_guard_interval:
					tn->param.u.ofdm.guard_interval = (fe_guard_interval_t)txt_to_ofdm_guard(token);
					break;
				case ofdm_hierarchy:
					tn->param.u.ofdm.hierarchy_information = (fe_hierarchy_t)txt_to_ofdm_hierarchy(token);
					count++;
					break;
				case atsc_modulation:
					tn->param.u.vsb.modulation = (fe_modulation_t)txt_to_atsc_mod(token);
					count++;
					break;
...
int vdr_code_from_token(const char * token) {
	...
	switch (toupper(*token)) {
		case 'S':
			...
			break;
		default:
			log_message(ERROR, "%s: could not parse %s",
				__FUNCTION__, token);
			exit(1);
    		}
		...
		switch (toupper(*token)) {
			...
			case '.':
				...
				if (!dot)
					pos *= 10;
				break;
             		default: log_message(ERROR, "%s: unknown source character '%c'",
					__FUNCTION__, *token);
							 exit(1);
			}
		}
	...
	while (fgets(buf, MAX_LINE_LENGTH, data) != NULL) {
		...
		switch (toupper(token[0])) {
			...
			case '#':
				continue;
			default:
				log_message(ERROR, "could not parse %s", positiondata);
				goto err;				
			}		

		while (NULL != (token = strtok(0, DELIMITERS))) {
			switch ((enum __rotor_args)arg++) {
				case rotor_position:
					switch (toupper(token[0])) {
						case '0' ... '9':
