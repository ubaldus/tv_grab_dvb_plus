...
void cTaskLoadepg::Action(void) {
	...
	case DATA_FORMAT_MHW_2:
		...
		bChannels = (unsigned char *) calloc(MAX_BUFFER_SIZE_CHANNELS,
				sizeof(unsigned char));
		if (!bChannels) {
			log_message(ERROR, "failed to allocate memory for bChannels");
			goto endrunning;
		}
		bTitles = (unsigned char *) calloc(MAX_BUFFER_SIZE_TITLES,
				sizeof(unsigned char));
		if (!bTitles) {
			log_message(ERROR, "failed to allocate memory for bTitles");
			goto endrunning;
		}
		bSummaries = (unsigned char *) calloc(MAX_BUFFER_SIZE_SUMMARIES,
				sizeof(unsigned char));
		if (!bSummaries) {
						...
						case '0':
							if (nH->P0 == NULL) {
								...
								if (nH->Value != NULL || (LenPrefix - 1) == i) {
									log_message(
											ERROR,
											"huffman prefix code already exists for \"%s\"=%s with '%s'",
				...
				if (sscanf(Line, "%c=%[^\n]\n", string1, string2) == 2) {
					...
					if (nH->Value != NULL) {
						if (memcmp(nH->Value, string1, strlen(nH->Value)) != 0) {
							log_message(
									ERROR,
									"huffman prefix value '%s' not equal to '%s'",
									nH->Value, string1);
						}
					} else {
						log_message(
								ERROR,
								"huffman prefix value is not exists for \"%s\"=%s",
								string1, string2);
					}
				}
			}
...
void cTaskLoadepg::AddFilter(unsigned short int Pid, unsigned char Tid,
		unsigned char Mask) {
	if (nFilters >= MAX_FILTERS) {
		log_message(
				ERROR,
				"numbers of filters is greater than %i, can't add filter pid=0x%04x tid=0x%02x",
				MAX_FILTERS, Pid, Tid);
		return;
	}
...
void cTaskLoadepg::StartFilter(int FilterId) {
	...
	if (ioctl(Filters[FilterId].Fd, DMX_SET_FILTER, &sctFilterParams) >= 0) {
		...
		log_message(ERROR, "can't starting filter pid=0x%04x tid=0x%02x",
				Filters[FilterId].Pid, Filters[FilterId].Tid);
...
void cTaskLoadepg::ReadBuffer(int FilterId, int Fd) {
	...
	if (Bytes < 0) {
		...
		case EOVERFLOW:
			log_message(ERROR,
					"buffer overflow to read filter for pid=0x%04x tid=0x%02x",
					Filters[FilterId].Pid, Filters[FilterId].Tid);
			break;
		case ETIMEDOUT:
			log_message(ERROR,
					"read filter timedout for pid=0x%04x tid=0x%02x",
					Filters[FilterId].Pid, Filters[FilterId].Tid);
			Filters[FilterId].Step = 2;
			break;
		default:
			log_message(ERROR,
					"failed to read filter for pid=0x%04x tid=0x%02x errno=%d",
					Filters[FilterId].Pid, Filters[FilterId].Tid, errno);
			Filters[FilterId].Step = 2;
			break;
		}
	} else {
...
void cTaskLoadepg::GetSatelliteTimeOffset(int FilterId, unsigned char *Data,
		int Length) {
	if (Data[0] == 0x73) {
		...
		while (DescriptorsLoopLength > 0) {
			...
			case 0x58:
				...
				log_message(DEBUG, "satellite time offset=[UTC]%+i",
						SatelliteTimeOffset / 3600);
				log_message(DEBUG, "EPG time offset=%+i seconds", EpgTimeOffset);
				if (is_logging(DEBUG)) {
					log_message(DEBUG, "satellite time UTC: %s %02i:%02i:%02i",
							GetStringMJD(satMJD), satH, satM, satS);
					log_message(DEBUG, "satellite country code=%s",
							SatelliteCountryCode);
					log_message(DEBUG, "satellite country region ID=%i",
							SatelliteCountryRegionId);
					log_message(DEBUG,
							"satellite local time offset polarity=%i",
							SatelliteTimeOffsetPolarity);
					log_message(DEBUG, "satellite local time offset=%02i:%02i",
							SatelliteTimeOffsetH, SatelliteTimeOffsetM);
				}
				break;
			default:
...
void cTaskLoadepg::SupplementChannelsSKYBOX(int FilterId, unsigned char *Data,
		int Length) {
	if (!EndBAT) {
		return;
	}
	...
	for (SI::Loop::Iterator it; sdt.serviceLoop.getNext(SiSdtService, it);) {
		...
		C = (sChannel *) bsearch(&Key, lChannels, nChannels, sizeof(sChannel),
				&bsearchChannelBySid);
		...
		for (SI::Loop::Iterator it2; (d
				= SiSdtService.serviceDescriptors.getNext(it2));) {
			...
			case SI::ServiceDescriptorTag: {
				...
				case 0x05: // NVOD time-shifted service
				{
					...
					sd->providerName.getText(ProviderNameBuf,
							sizeof(ProviderNameBuf));
			...
			case SI::MultilingualServiceNameDescriptorTag: {
				if (C == NULL)
					break;
				SI::MultilingualServiceNameDescriptor * md =
						(SI::MultilingualServiceNameDescriptor *) d;
...
void cTaskLoadepg::GetChannelsSKYBOX(int FilterId, unsigned char *Data,
		int Length) {
	...
	if (Data[0] == 0x4a) {
		...
		while (TransportStreamLoopLength > 0) {
			...
			int TransportDescriptorsLength = ((Data[p1 + 4] & 0x0f) << 8)
					| Data[p1 + 5];
			int p2 = (p1 + 6);
...
void cTaskLoadepg::GetTitlesSKYBOX(int FilterId, unsigned char *Data,
		int Length) {
	int p;
	...
	if (memcmp(&InitialBuffer[FilterId][0], Data, 20) == 0) {
		...
		if (ChannelId > 0) {
			if (MjdTime > 0) {
				...
				if (Data[p + 4] != 0xb5) {
					log_message(WARNING,
							"data error signature for title (0x%d)",
							Data[p + 4]);
					goto endloop1;
				}
				if (Len1 > Length) {
				...
				if ((pT + Len2 + 2) > MAX_BUFFER_SIZE_TITLES) {
					log_message(ERROR,
							"buffer overflow, titles size more than %i bytes",
							MAX_BUFFER_SIZE_TITLES);
					IsError = true;
					return;
				}
...
void cTaskLoadepg::GetSummariesSKYBOX(int FilterId, unsigned char *Data,
		int Length) {
	int p;
	unsigned short int ChannelId;
	unsigned short int MjdTime;
	...
	if (memcmp(&InitialBuffer[FilterId][0], Data, 20) == 0) {
		...
		if (ChannelId > 0) {
			if (MjdTime > 0) {
				...
				if (Data[p + 4] != 0xb9) {
					log_message(WARNING,
							"data error signature for summary (0x%x)", Data[p
									+ 4]);
					//goto endloop1;
				}
				if (Len1 > Length) {
				...
				if ((pS + Len2 + 2) > MAX_BUFFER_SIZE_SUMMARIES) {
					log_message(
							ERROR,
							"buffer overflow, summaries size more than %i bytes",
				...
				if (nSummaries >= MAX_SUMMARIES) {
					log_message(ERROR, "summaries found more than %i",
							MAX_SUMMARIES);
					IsError = true;
					return;
				}
...
void cTaskLoadepg::GetChannelsMHW1(int FilterId, unsigned char *Data,
		int Length) {
	...
	if (nChannels > MAX_CHANNELS) {
		...
		for (int i = 0; i < nChannels; i++) {
			...
			if ((pC + 18) > MAX_BUFFER_SIZE_CHANNELS) {
				log_message(ERROR,
						"buffer overflow, channels size more than %i bytes",
						MAX_BUFFER_SIZE_CHANNELS);
				IsError = true;
				return;
			}
		...
		if (Title->ChannelId != 0xff) {
			if (nTitles < MAX_TITLES) {
				if (memcmp(&InitialBuffer[FilterId][0], Data, 46) == 0) {
					...
					int StartTime = (Day * 86400) + (Hours * 3600) + (Minutes
							* 60);
...
void cTaskLoadepg::GetSummariesMHW1(int FilterId, unsigned char *Data,
		int Length) {
	sSummaryMHW1 *Summary = (sSummaryMHW1 *) Data;
	if (Length > 11) {
		if (Summary->NumReplays < 10) {
			if (Length > (11 + (Summary->NumReplays * 7))) {
				if (Summary->Byte7 == 0xff && Summary->Byte8 && Summary->Byte9
						== 0xff) {
					if (nSummaries < MAX_SUMMARIES) {
						if (memcmp(&InitialBuffer[FilterId][0], Data, 20) == 0) {
							Filters[FilterId].Step = 2;
							...
							memcpy(&bSummaries[pS], &Data[SummaryOffset],
									SummaryLength);
		...
		for (int i = 0; i < Data[4]; i++) {
			...
			if (Length > p1) {
				for (int ii = 0; ii <= (Data[p1] & 0x3f); ii++) {
					p2 = ((Data[p1 + 1 + ii * 2] << 8) | Data[p1 + 2 + ii * 2])
							+ 3;
					if (Length > p2) {
						if (ii == 0) {
							pThemeName = p2 + 1;
						...
						if (Length >= (pThemeName + lenThemeName)) {
							...
							if ((lenThemeName + 2) < 256) {
								memcpy((lThemes + pThemeId)->Name,
										&Data[pThemeName], lenThemeName);
								if (Length >= (pSubThemeName + lenSubThemeName)) {
									if (lenSubThemeName > 0) {
										if ((lenThemeName + lenSubThemeName + 2)
												< 256) {
...
void cTaskLoadepg::GetChannelsMHW2(int FilterId, unsigned char *Data,
		int Length) {
	if (EndThemes && EndChannels) {
		Filters[FilterId].Step = 2;
		return;
	...
	if (Length > 18) {
		...
		if (memcmp(&InitialBuffer[FilterId][0], Data, 16) == 0) {
			...
			while (Pos < Length) {
				...
				T->StartTime = ((T->MjdTime - 40587) * 86400) + (((((Data[Pos
						+ 5] & 0xf0) >> 4) * 10) + (Data[Pos + 5] & 0x0f))
						* 3600) + (((((Data[Pos + 6] & 0xf0) >> 4) * 10)
						+ (Data[Pos + 6] & 0x0f)) * 60);
				T->Duration = (((Data[Pos + 8] << 8) | Data[Pos + 9]) >> 4)
						* 60;
...
void cTaskLoadepg::GetSummariesMHW2(int FilterId, unsigned char *Data,
		int Length) {
	if (Length > (Data[14] + 17)) {
		if (memcmp(&InitialBuffer[FilterId][0], Data, 16) == 0) {
			...
			if ((pS + lenText + 2) < MAX_BUFFER_SIZE_SUMMARIES) {
				...
				if (Loop > 0) {
					while (Loop > 0) {
						...
						if ((Pos + lenText) < Length) {
							...
							memcpy(&bSummaries[S->pData + lenSummary],
									&Data[Pos], lenText);
							lenSummary += lenText;
							if (Loop > 1) {
								bSummaries[S->pData + lenSummary] = '|';
				...
				log_message(ERROR,
						"buffer overflow, summaries size more than %i",
						MAX_BUFFER_SIZE_SUMMARIES);
				IsError = true;
				return;
			}
...
void cTaskLoadepg::CreateEpgXml(void) {
	...
	if ((format == DATA_FORMAT_SKY_AU) || (format == DATA_FORMAT_SKY_IT)
				...
				if (nTitles > 0) {
					if (nSummaries > 0) {
						...
						while (i < nTitles) {
							...
							C = (sChannel *) bsearch(&KeyC, lChannels,
									nChannels, sizeof(sChannel),
									&bsearchChannelByChannelId);
							if (C) {
								C->IsFound = true;
							}

							StartTime = (T->StartTime + EpgTimeOffset);
							StopTime = (T->StartTime + T->Duration
									+ EpgTimeOffset);
							 ...
							 */
							if ((StartTime >= start_of_period) && (StartTime
									< end_of_period)) {
								...
								if (channelIdent == NULL) {
									...
									if (DecodeHuffmanCode(&bTitles[T->pData],
											T->lenData)) {
										...
										printf(
												...
												"en",
												xmlify(
														(const char *) DecodeText));
									}
									sSummary KeyS, *S;
									KeyS.ChannelId = ChannelId;
									KeyS.MjdTime = T->MjdTime;
									KeyS.EventId = T->EventId;
									S = (sSummary *) bsearch(&KeyS, lSummaries,
											nSummaries, sizeof(sSummary),
											&bsearchSummarie);
									if (S) {
										if (DecodeHuffmanCode(
												&bSummaries[S->pData],
												S->lenData)) {
											CleanString(DecodeText);
											char *d = xmlify(
		...
		while (i < nTitles) {
			...
			if (ChannelId != T->ChannelId) {
				...
				C = (sChannel *) bsearch(&KeyC, lChannels, nChannels,
						sizeof(sChannel), &bsearchChannelByChannelId);
				ChannelId = T->ChannelId;
			}
			if (IsChannel) {
				log_message(TRACE, "E %u %u %u 01 FF", T->EventId,
						(T->StartTime + EpgTimeOffset), T->Duration);
				...
				if ((lThemes + T->ThemeId)->Name[0] != '\0') {
					if (is_logging(TRACE)) {
						...
						log_message(TRACE, "S %s - %d\' - %s", (lThemes
								+ T->ThemeId)->Name, T->Duration / 60, DateTime);
					}
				}
				sSummary KeyS, *S;
				KeyS.ChannelId = 0;
				KeyS.MjdTime = 0;
				KeyS.EventId = T->EventId;
				S = (sSummary *) bsearch(&KeyS, lSummaries, nSummaries,
						sizeof(sSummary), &bsearchSummarie);
				if (S) {
